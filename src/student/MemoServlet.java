	...
	public void add(HttpServletRequest request, HttpServletResponse response)
		...
		if (over) {
			...
			if (null == session.getAttribute("id")) {
				...
				f.setCreator(su.toGBK(session.getAttribute("id")
						.toString())); // 获取并保存创建者
...
System.out.println("查看备忘信息时的URL："+url);
			List<MemoForm> list = memoDAO.query(session.getAttribute("id").toString(),y, m, d,week);
	...
	public void listAll(HttpServletRequest request, HttpServletResponse response)
		...
		if (null == session.getAttribute("id")) {
			...
			List<MemoForm> list = memoDAO.query(session.getAttribute("id").toString());
			request.setAttribute("memoList", list); // 保存备忘信息
			request.setAttribute("url", url);		//保存打开页面的URL地址
	...
	List<MemoForm> list = memoDAO.query(session.getAttribute("id").toString(),key);
	...
	public void getRemind(HttpServletRequest request,
		...
		String content="";
		HttpSession session = request.getSession();
		content=memoDAO.getRemind(session.getAttribute("id").toString());		//查询到期提醒的备忘信息
